...
<Card title="Daytona Workspace" icon="rocket" href="#daytona">
  Isolate tool execution in lightning-fast, secure Daytona sandboxes.
</Card>
...
To use Daytona workspaces, install the Daytona workspace extension:
```bash
pip install composio-core[daytona]
```
...
toolset = ComposioToolSet(workspace_config=WorkspaceType.Daytona(
    api_key="your_daytona_api_key",  # Required: Daytona API Key
    language="typescript",  # Optional: Programming language for the sandbox (default: 'python')
    id="sandbox_id",  # Optional: Custom identifier for the sandbox
    name="sandbox_name", # Optional: Custom display name for the sandbox
    image="your-custom-image:tag", # Optional: Custom Docker image to use for the sandbox
    env_vars={"KEY": "VALUE"},  # Optional: Environment variables to set in the sandbox
    labels={"KEY": "VALUE"}, # Optional: Custom labels for the sandbox
    public=True, # Optional: Whether the sandbox should be public
    target="eu", # Optional: Target location for the sandbox (default: 'us')
    resources={"cpu": 2, "memory": 4}, # Optional: Resource configuration for the sandbox
    auto_stop_interval=5, # Optional: Interval in minutes after which the sandbox will automatically stop if no event occurs during that time, 0 means no auto-stop (default: 15 minutes)
))
tools = toolset.get_tools(["FILETOOL_LIST_FILES"])
...
response = client.chat.completions.create(
    ...
    max_tokens=1024,
    tools=tools,
    messages=[{"role": "user", "content": "List files in Daytona sandbox"}],
)
result = toolset.handle_tool_calls(response)
print(result)
```
